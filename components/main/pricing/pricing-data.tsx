import React from "react";
import { PricingFeature } from "./PricingCard";

export const starterFeatures: PricingFeature[] = [
  {
    id: 1,
    icon: "https://www.fitplay.life/_next/image?url=https%3A%2F%2Fu1dfrpi3ij.ufs.sh%2Ff%2FWeAa8lScfGDEqaVsTNICdMUXY9x7ADl21anWvH08eJ3okrEO&w=828&q=75",
    title: "Probiotic Deficiency Test",
    description: "report in 21 days",
  },
  {
    id: 2,
    icon: "/packs/gut_report.png",
    title: "Gut Score & Detailed Report Card",
    description: "",
  },
  {
    id: 3,
    icon: "https://www.fitplay.life/_next/image?url=https%3A%2F%2Fcdn.shopify.com%2Fs%2Ffiles%2F1%2F0687%2F4523%2F2705%2Ffiles%2FConsultOldPDP.png%3Fv%3D1743995627&w=828&q=75",
    title: "1:1 counselling by a Gut expert",
    description: "to decode your report",
  },
  {
    id: 4,
    icon: "https://images.unsplash.com/photo-1595871561397-7f946e695bc6?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxkaWV0JTIwcGxhbiUyMG5vdGVib29rJTIwaGVhbHRoeXxlbnwxfHx8fDE3NzIwMDgzMjV8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
    title: "30-day diet plan",
    description: "& dedicated support by a certified clinical nutritionist",
  },
  {
    id: 5,
    icon: "/packs/glass_bottle.jpg",
    title: "Motivational Personal Glass bottle",
    description: "Hydration reminder Water bottle",
  },
  {
    id: 6,
    icon: "https://images.unsplash.com/photo-1673515335586-f9f662c01482?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxvbmxpbmUlMjBsZWFybmluZyUyMHdlbGxuZXNzJTIwbW9kdWxlc3xlbnwxfHx8fDE3NzIwMDgzMjd8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
    title: "Self learning modules",
    description: "for lifestyle management & Gut health improvement",
  },
];

export const eliteFeatures: PricingFeature[] = [
  {
    id: 1,
    icon: "https://www.fitplay.life/_next/image?url=https%3A%2F%2Fu1dfrpi3ij.ufs.sh%2Ff%2FWeAa8lScfGDEqaVsTNICdMUXY9x7ADl21anWvH08eJ3okrEO&w=828&q=75",
    title: "Probiotic Deficiency Test",
    description: "21 days",
  },
  {
    id: 2,
    icon: "/packs/gut_report.png",
    title: "Gut Score & Detailed Report Card",
    description: "",
  },
  {
    id: 3,
    icon: "https://www.fitplay.life/_next/image?url=https%3A%2F%2Fcdn.shopify.com%2Fs%2Ffiles%2F1%2F0687%2F4523%2F2705%2Ffiles%2FConsultOldPDP.png%3Fv%3D1743995627&w=828&q=75",
    title: "1:1 counselling by a Gut expert",
    description: "to decode your report",
  },
  {
    id: 4,
    icon: "https://images.unsplash.com/photo-1595871561397-7f946e695bc6?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxkaWV0JTIwcGxhbiUyMG5vdGVib29rJTIwaGVhbHRoeXxlbnwxfHx8fDE3NzIwMDgzMjV8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
    title: "60-day diet plan",
    description: "& dedicated support by a certified clinical nutritionist",
  },
  {
    id: 5,
    icon: "https://images.unsplash.com/photo-1760088348194-a5ac70a8aa9f?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxzbWFydCUyMHJpbmclMjBmaXRuZXNzJTIwdHJhY2tlcnxlbnwxfHx8fDE3NzIwMDgzMjZ8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
    title: "BOAT Active Plus Smart Ring",
    description: "for lifestyle management",
  },
  {
    id: 6,
    icon: "/packs/glass_bottle.jpg",
    title: "Motivational Personal Glass bottle",
    description: "Hydration reminder Water bottle",
  },
  {
    id: 7,
    icon: "https://images.unsplash.com/photo-1673515335586-f9f662c01482?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxvbmxpbmUlMjBsZWFybmluZyUyMHdlbGxuZXNzJTIwbW9kdWxlc3xlbnwxfHx8fDE3NzIwMDgzMjd8MA&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral",
    title: "Self learning modules",
    description: "for lifestyle management & Gut health improvement",
  },
];
