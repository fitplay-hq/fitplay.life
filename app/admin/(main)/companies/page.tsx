...
function EmployeesDrawer({ company, users, open, onOpenChange, mutateAll }: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={MODAL_CLS}>
        <DialogHeader className="px-8 pt-7 pb-4 border-b shrink-0">
          <DialogTitle className="text-xl font-bold">
            Employees — {company?.name}
          </DialogTitle>
          <DialogDescription>
            {companyUsers.length} employee{companyUsers.length !== 1 ? "s" : ""}
          </DialogDescription>
        </DialogHeader>

        {/* Scrollable area — only vertical */}
          ...
          <div className="border rounded-xl overflow-hidden shadow-sm w-full">
            <table className="w-full table-fixed text-sm">
              <thead className="bg-gray-50 border-b">
                <tr>
                  <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[20%]">Name</th>
                  <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[28%]">Email</th>
                ...
                {companyUsers.length > 0 ? (
                  companyUsers.map((u: any) => (
                    <tr key={u.id} className="border-b last:border-0 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-gray-600 truncate">{u.email}</td>
        ...
        <DialogFooter className="px-8 py-4 border-t bg-gray-50 shrink-0">
          <DialogClose asChild>
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setOrders(
          (data.orders || []).filter((o: any) => o.user?.companyId === company?.id)
        );
      }
    } catch {
      toast.error("Failed to fetch orders");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={MODAL_CLS}>
        <DialogHeader className="px-8 pt-7 pb-4 border-b shrink-0">
          ...
          <DialogDescription>{orders.length} order{orders.length !== 1 ? "s" : ""}</DialogDescription>
        </DialogHeader>
          ...
          {loading ? (
            ...
            <div className="border rounded-xl overflow-hidden shadow-sm w-full">
              <table className="w-full table-fixed text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[14%]">Order ID</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[22%]">Employee</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[16%]">Amount</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[10%]">Items</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[18%]">Status</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[20%]">Date</th>
                  </tr>
                </thead>
                <tbody>
                  {orders.length > 0 ? (
                    orders.map((o: any) => (
                      <tr key={o.id} className="border-b last:border-0 hover:bg-gray-50">
                        <td className="px-4 py-3 font-mono truncate">{o.id.slice(0, 8)}</td>
                        <td className="px-4 py-3 truncate">{o.user?.name}</td>
...
function ProductsDrawer({ company, open, onOpenChange }: any) {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        setProducts(
          (data.data || data.products || []).map((p: any) => ({
            ...p,
            companies: p.companies || [],
            variants: p.variants || [],
            companyIds: (p.companies || [])
              .map((c: any) =>
                c?.id || c?._id || (typeof c === "string" ? c : null)
              )
              .filter(Boolean),
          }))
        );
  ...
  const updateProductVisibility = async (productId: string, enable: boolean) => {
    try {
      const product = products.find((p) => p.id === productId);
      if (!product) return;
      const existingIds = product.companyIds || [];
      const newIds = enable
        ? Array.from(new Set([...existingIds, company.id]))
        : existingIds.filter((id: string) => id !== company.id);

      const res = await fetch("/api/prod-visiblity", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ productId, companyIds: newIds }),
      });
      if (!res.ok) throw new Error("Failed to update visibility");
      setProducts((prev) =>
        prev.map((p) => (p.id === productId ? { ...p, companyIds: newIds } : p))
      );
      toast.success(`Product ${enable ? "enabled" : "disabled"}`);
    } catch {
      toast.error("Failed to update product visibility");
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className={MODAL_CLS}>
        <DialogHeader className="px-8 pt-7 pb-4 border-b shrink-0">
          <DialogTitle className="text-xl font-bold">
            Product Visibility — {company?.name}
          </DialogTitle>
          <DialogDescription>
        ...
        {/* Filter bar */}
        <div className="px-8 py-3 border-b bg-gray-50 shrink-0">
          <div className="flex gap-3">
            ...
            <select
              value={selectedCategory}
              onChange={(e) => setSelectedCategory(e.target.value)}
              className="px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-emerald-500 bg-white text-sm w-48"
          ...
          {loading ? (
            ...
            <div className="border rounded-xl overflow-hidden shadow-sm w-full">
              <table className="w-full table-fixed text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[7%]">Img</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[26%]">Product</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[18%]">Category</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[16%]">Vendor</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[13%]">Credits</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[12%]">Visible</th>
                    <th className="text-right px-4 py-3 font-semibold text-gray-600 w-[8%]">Toggle</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={product.id} className="border-b last:border-0 hover:bg-gray-50">
                        <td className="px-4 py-2">
                          <div className="w-11 h-11 overflow-hidden bg-gray-100 rounded-lg shrink-0">
                            <ImageWithFallback
                              src={product?.images?.[0] || "/placeholder.png"}
                              alt={product?.name || "Product"}
                              className="w-full h-full object-cover"
                        ...
                        <td className="px-4 py-2">
                          <div className="flex items-center gap-1">
                            ...
                            <span className="font-medium text-amber-700">{credit || "N/A"}</span>
                          </div>
                        </td>
                        <td className="px-4 py-2">
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className={`text-xs font-medium ${isVisible ? "text-green-700" : "text-gray-400"}`}>
                              {isVisible ? "On" : "Off"}
                            </span>
                          </div>
                        </td>
                        <td className="px-4 py-2 text-right">
                          <button
                            onClick={() =>
                              updateProductVisibility(product?.id || "", !isVisible)
                            }
                            style={{ width: "44px" }}
          ...
          {loading ? (
            ...
            <div className="border rounded-xl overflow-hidden shadow-sm w-full">
              <table className="w-full table-fixed text-sm">
                <thead className="bg-gray-50 border-b">
                  <tr>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[16%]">Txn ID</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[22%]">Employee</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[16%]">Amount</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[20%]">Type</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[14%]">Status</th>
                    <th className="text-left px-4 py-3 font-semibold text-gray-600 w-[12%]">Date</th>
                  </tr>
                </thead>
                <tbody>
                  {transactions.length > 0 ? (
                    transactions.map((t: any) => (
                      <tr key={t.id} className="border-b last:border-0 hover:bg-gray-50">
                        <td className="px-4 py-3 font-mono truncate">{t.id.slice(0, 8)}</td>
                        <td className="px-4 py-3 truncate">{t.user?.name}</td>
...
function CompanyDialog({ open, onOpenChange, company, onSave }: any) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[80vw] max-w-2xl bg-white p-0">
        ...
        <div className="px-8 py-6 space-y-5">
          {[
            { id: "comp-name", label: "Company Name *", key: "name", placeholder: "Enter company name" },
            { id: "comp-address", label: "Address *", key: "address", placeholder: "Enter company address" },
        ...
        <DialogFooter className="px-8 py-4 border-t bg-gray-50 gap-3">
          <Button variant="outline" size="lg" onClick={() => onOpenChange(false)}>Cancel</Button>
          <Button onClick={handleSave} disabled={loading} size="lg" className="bg-emerald-600 hover:bg-emerald-700">
            {loading ? "Updating…" : "Save Changes"}
          </Button>
        </DialogFooter>
      </DialogContent>
...
export default function CompaniesManagementPage() {
  ...
  const products     = (productsRes?.data || productsRes?.products || []).map((p: any) => ({
    ...
    companies: p.companies || [],
    companyIds: (p.companies || [])
      .map((c: any) => c?.id || c?._id || c?.companyId || (typeof c === "string" ? c : null))
      .filter(Boolean),
  }));
  ...
  const companyMetrics = (company: any) => {
    ...
    return {
      ...
      transactions: companyTx.length,
      totalSpent: companyOrders.reduce((s: number, o: any) => s + (o.amount || 0), 0),
    };
  };
  ...
  const openDrawer = (company: any, type: string) => {
    setSelectedCompany(company);
    if (type === "employees")    setEmployeesDrawerOpen(true);
    if (type === "orders")       setOrdersDrawerOpen(true);
    if (type === "products")     setProductsDrawerOpen(true);
    if (type === "transactions") setTransactionsDrawerOpen(true);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Company Management</h1>
          <p className="text-gray-600 mt-1">Manage all companies, employees, orders, and products</p>
        </div>
        <div className="flex items-center gap-3">
          <Dialog open={createCompanyOpen} onOpenChange={setCreateCompanyOpen}>
            <DialogTrigger asChild>
            ...
            <DialogContent className="w-[80vw] max-w-2xl bg-white p-0">
              <DialogHeader className="px-8 pt-7 pb-4 border-b">
                <DialogTitle className="text-xl font-bold">Create Company</DialogTitle>
              ...
              <div className="px-8 py-6 space-y-5">
                <div className="space-y-2">
                  ...
                  <Input id="company-name" value={companyForm.name} onChange={(e) => setCompanyForm({ ...companyForm, name: e.target.value })} placeholder="Enter company name" className="h-11" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="company-address" className="text-sm font-semibold">Address *</Label>
                  <Input id="company-address" value={companyForm.address} onChange={(e) => setCompanyForm({ ...companyForm, address: e.target.value })} placeholder="Enter company address" className="h-11" />
                </div>
              </div>
              <DialogFooter className="px-8 py-4 border-t bg-gray-50 gap-3">
                <Button variant="outline" size="lg" onClick={() => setCreateCompanyOpen(false)}>Cancel</Button>
                <Button onClick={handleCreateCompany} disabled={creatingCompany} size="lg" className="bg-emerald-600 hover:bg-emerald-700">
                  {creatingCompany ? "Creating…" : "Create Company"}
                </Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
          <button onClick={() => mutate("/api/companies")} className="p-2 hover:bg-gray-100 rounded-lg transition-colors" aria-label="Refresh">
            <RefreshCw className="w-4 h-4 text-gray-500" />
          </button>
        </div>
      </div>
        ...
        {[
          ...
          <Card key={label} className="border border-gray-200 hover:shadow-lg transition-shadow">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              <div className={`p-2 rounded-lg ${bg}`}><Icon className={`w-4 h-4 ${ic}`} /></div>
            </CardHeader>
            <CardContent>
    ...
    0
                    ...
                    return (
                      <tr key={company.id} className="border-b last:border-0 hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 text-gray-500 truncate">{company.address || "—"}</td>
                        ...
                        <td className="px-6 py-4 text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end" className="w-56">
                              <DropdownMenuItem onClick={() => openDrawer(company, "employees")}><Users className="h-4 w-4 mr-2" />Manage Employees</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => openDrawer(company, "orders")}><ShoppingCart className="h-4 w-4 mr-2" />View Orders</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => openDrawer(company, "products")}><Package className="h-4 w-4 mr-2" />Product Visibility</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => openDrawer(company, "transactions")}><CreditCard className="h-4 w-4 mr-2" />View Transactions</DropdownMenuItem>
                              <div className="my-1 border-t" />
                              <DropdownMenuItem onClick={() => { setSelectedCompany(company); setEditCompanyOpen(true); }}><Edit2 className="h-4 w-4 mr-2" />Edit Company</DropdownMenuItem>
                              <DropdownMenuItem className="text-red-600" onClick={() => handleDeleteCompany(company.id)}><Trash2 className="h-4 w-4 mr-2" />Delete Company</DropdownMenuItem>
                            </DropdownMenuContent>
                          </DropdownMenu>
                        </td>
                      </tr>
      ...
      {/* Modals */}
      <EmployeesDrawer company={selectedCompany} users={users} open={employeesDrawerOpen} onOpenChange={setEmployeesDrawerOpen}
        mutateAll={() => { mutate("/api/admin/users"); mutate("/api/companies"); }} />
      <OrdersDrawer company={selectedCompany} open={ordersDrawerOpen} onOpenChange={setOrdersDrawerOpen} />
      <ProductsDrawer company={selectedCompany} open={productsDrawerOpen} onOpenChange={setProductsDrawerOpen} />
      <TransactionsDrawer company={selectedCompany} open={transactionsDrawerOpen} onOpenChange={setTransactionsDrawerOpen} />
      <CompanyDialog open={editCompanyOpen} onOpenChange={setEditCompanyOpen} company={selectedCompany} onSave={() => mutate("/api/companies")} />
    </div>
  );
}
